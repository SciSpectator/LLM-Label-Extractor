...
# Extraction prompt — same format as proven working old script
# METADATA inline + JSON SCHEMA at end — tested and working with gemma2 models
# ─────────────────────────────────────────────────────────────────────────────
# EXTRACTION AGENT PROMPT  (gemma2:2b, Phase 1)
# Purpose: READ the raw GEO text and COPY the values out as-is.
# No reasoning, no vocabulary, no normalisation — just structured extraction.
# The model must NOT invent, generalise, or map to any controlled vocabulary.
# Specificity rule: copy the most specific term present in the text.
# ─────────────────────────────────────────────────────────────────────────────
...
# NS INFERENCE PROMPT — used AFTER raw extraction for fields still Not Specified.
# Different from extraction: this INFERS from experiment context, not copies from sample.
...
class MemoryAgent:
    ...
    def is_cluster_name(self, col: str, label: str) -> bool:
        """
        Returns True if label is an exact cluster name in LLM_memory.
        Used by the cluster gate to validate resolved labels.
        """
        try:
            with self._conn() as c:
                row = c.execute(
...
EXTRACTION_MODEL   = "gemma2:2b"    # fast extraction model (Phase 1 + Step 1a)
...
class GSEInferencer:
    """
    Phase 1b — Per-GSE inference agent with KV cache reuse.

    Context : GSE experiment metadata as SYSTEM prompt (cached by Ollama KV)
              → only the USER message (sample metadata) changes per sample
    Memory  : Deterministic (GSE experiment description — static per GSE)
    Tools   : None
    Model   : gemma2:2b

    One instance per GSE. System prompt KV cached → ~40% faster.
    """
    ...
    def __init__(self, gse_id: str, gse_meta: dict, ollama_url: str,
        ...
        self._system = (
            "You are a biomedical metadata annotator. This sample is part of "
            f"the following experiment:\n\n"
            f"EXPERIMENT TITLE: {_title}\n"
            f"EXPERIMENT SUMMARY: {_summary}\n"
            f"EXPERIMENT DESIGN: {_design}\n\n"
            "Based on this experiment context, INFER the missing fields. "
            "If the experiment studies a disease, the sample has that condition. "
            "If it uses a specific tissue, the sample is from that tissue. "
            "Control/healthy in disease study → Condition = Control. "
            "Only infer from context. Unknown = Not Specified. "
            "Output JSON only: {\"Tissue\":\"\", \"Condition\":\"\", \"Treatment\":\"\"}"
        )

    def _llm_call(self, user_msg: str, max_tokens: int = 200) -> str:
        if self.watchdog:
            self.watchdog.wait_if_paused()
            self.watchdog.record_call()
        messages = [
    ...
    def infer_sample(self, gsm: str, raw: dict,
                     ...
                     cols: list = None) -> Dict[str, str]:
        """Infer missing labels for one sample. Only re-infers NS fields."""
        _cols = cols or LABEL_COLS_SCRATCH
        ...
        user_msg = (f"Title: {_title}\nSource: {_source}\n"
                    ...
                    f"Fields still missing: {', '.join(ns_fields)}\nJSON:")
        text = ""
        for attempt in range(3):
            text = self._llm_call(user_msg)
            if text: break
            time.sleep(5 * (attempt + 1))
        inferred = _parse_json_extraction(text, _cols)
...
class CollapseWorker:
    """
    Phase 2 — Per-sample collapse agent with 4-tier memory.

    Context : GSEContext (sibling label distribution — static)
              + raw sample metadata + Phase 1/1b extracted label
    Memory  :
      Deterministic (no LLM):
        · Tier 0: Cluster Map — O(1) raw→canonical lookup
        · Tier 4: KG triples (synonym/variant)
        · GSE-dominant fast path (>70%)
        · Deterministic fallback (exact_match, abbreviation)
      Agent-augmented (LLM ReAct loop):
        · Tier 2: Semantic embeddings (nomic-embed-text cosine search)
        · Tier 3: Episodic memory (past resolutions)
        · ReAct agent: SEARCH / PICK / NEW_CLUSTER tools, max 3 turns
    Model   : gemma2:2b

    Fully parallel per (gsm, col). No per-GSE sequential bottleneck.
    """
    ...
    def _llm_chat(self, messages: List[dict], max_tokens: int = 200) -> str:
        """Multi-turn chat for ReAct agent loop."""
        if self.watchdog:
            self.watchdog.wait_if_paused()
            self.watchdog.record_call()
        if _OLLAMA_LIB_OK:
            ...
            return ""
        url = self.url.rstrip("/") + "/api/chat"
        payload = {"model": self.model, "stream": False, "keep_alive": -1,
    ...
    # ── ReAct agent (LLM-augmented) ──────────────────────────────────────

    def _run_react(self, gsm, col, out1, ctx_labels, ctx_counts,
                   gse_block, raw) -> tuple:
        """ReAct collapse agent — 3 tools, max 3 turns."""
        ma = self.mem_agent
        gsm_block = format_raw_block(raw) if raw else "(no metadata)"

        def tool_search(query):
            if not ma or not ma.is_ready(col):
                return "OBSERVATION: memory not available"
            hits = ma.semantic_search(col, query.strip(), k=10)
            d = ma.cluster_lookup(col, query.strip())
            if d and not any(l == d for l, _ in hits):
                hits = [(d, 1.0)] + hits
            ranked = _rank_candidates_by_specificity(query.strip(), hits)
            if not ranked:
        ...
        def tool_pick(cn):
            cn = cn.strip().rstrip(".")
            if ma and ma.is_cluster_name(col, cn): return cn, True, "react_pick"
            if ma:
                r = ma.cluster_lookup(col, cn)
                if r and ma.is_cluster_name(col, r): return r, True, "react_pick+remap"
            return None, False, "react_pick_invalid"
        ...
        def tool_new(cn):
            ...
            ok = (len(cn) >= 4 and not is_ns(cn) and
                  not any(w in cn.lower() for w in
                          ("unknown","unspecified","n/a","none","other","not ","mixed")))
            if not ok: return None, False, "new_rejected"
            if ma: ma.register_new_cluster(col, cn, out1, self._log)
            return cn, True, "react_new_cluster"

        # Pre-load
        ep = self.mem_agent.episodic_search(col, out1) if ma else []
        ep_str = "\n".join(
            f"  → {e['canonical']!r} (×{e['count']}, conf={e['confidence']:.2f})"
            for e in (ep or [])[:5]) or "none"
        seed = out1
        if is_ns(out1) and ctx_counts:
            seed = max(ctx_counts, key=ctx_counts.get)
        init_search = tool_search(seed)

        gse_ctx_str = gse_block or ""
        if ctx_counts:
            gse_ctx_str += f"\nSibling labels for {col}:\n"
            gse_ctx_str += "\n".join(f"  {l!r}: {c}"
                for l, c in sorted(ctx_counts.items(), key=lambda x: -x[1])[:15])

        sys_p = (
            f"You are a biomedical label collapse agent for: {col}.\n"
            f"Map the label to the MOST SPECIFIC cluster, or NEW_CLUSTER if unique.\n"
            f"TOOLS: SEARCH: <q> | PICK: <name> | NEW_CLUSTER: <name>\n"
            f"RULES: 1) Specificity first. 2) Cell type ≠ organ. "
            f"3) Vague labels → PICK: NO_MATCH\n"
            f"FORMAT: THOUGHT: ... then ACTION: TOOL: value"
        )
        _hint = " (from raw text)" if not is_ns(out1) else " (NS — use GSE context)"
        ctx = (f"EXPERIMENT:\n{gse_ctx_str}\n\n"
               f"SAMPLE {gsm}: {gsm_block[:300]}\n\n"
               f"PAST RESOLUTIONS:\n{ep_str}\n\n"
               f"LABEL: {out1!r}{_hint}\n\n"
               f"{init_search}\n\nTHOUGHT:")
        msgs = [{"role": "system", "content": sys_p},
                {"role": "user",   "content": ctx}]

        for _ in range(3):
            reply = self._llm_chat(msgs, 200)
            if not reply: break
            msgs.append({"role": "assistant", "content": reply})
            act = ""
            for line in reply.splitlines():
                s = line.strip().upper()
                if s.startswith("ACTION:"): act = line.strip()[7:].strip(); break
                for t in ("PICK:", "NEW_CLUSTER:", "SEARCH:"):
                    if s.startswith(t): act = line.strip(); break
                if act: break
            if not act: continue
            au = act.upper()
            if au.startswith("PICK:"):
                r, ok, rule = tool_pick(act[5:].strip())
                if ok: return r, True, rule
                msgs.append({"role": "user", "content":
                    f"OBSERVATION: not a valid cluster. Try SEARCH."})
            elif au.startswith("NEW_CLUSTER:"):
                r, ok, rule = tool_new(act[12:].strip())
                if ok: return r, True, rule
                msgs.append({"role": "user", "content":
                    f"OBSERVATION: rejected. Try SEARCH or PICK."})
            elif au.startswith("SEARCH:"):
                msgs.append({"role": "user", "content": tool_search(act[7:].strip())})
            else: break
    ...
    def collapse_field(self, gsm: str, col: str, raw_label: str,
        ...
        #    of cluster names. e.g. "Ds" → "Down Syndrome", "AD" → "Alzheimer Disease"
        ...
        # 3. ReAct agent
        if not collapsed and ma and ma.is_ready(col):
            gse_block = ""
            if gse_ctx:
                parts = []
                if gse_ctx.title: parts.append(f"Title: {gse_ctx.title}")
                if gse_ctx.summary: parts.append(f"Summary: {gse_ctx.summary[:500]}")
                if gse_ctx.design: parts.append(f"Design: {gse_ctx.design[:300]}")
                gse_block = "\n".join(parts) + "\n" if parts else ""
            final, collapsed, rule = self._run_react(
                gsm, col, out1, ctx_labels, ctx_counts, gse_block, raw or {})
        ...
        if _has_vocab:
            ...
            # Treatment: snap to dominant sibling form
...
class GSEWorker:
    ...
    def _llm_with_model(self, prompt: str, model: str,
        ...
        """Call LLM with a specific model override (e.g. gemma2:2b for extraction).
        Falls back to self.model if the specified model is not available.
        """
        if self.watchdog:
            self.watchdog.wait_if_paused()
            self.watchdog.record_call()
        ...
        if system:
            messages.append({"role": "system", "content": system})
        messages.append({"role": "user", "content": prompt})
        ctx_size = 1024  # extraction with full GSE context + treatment + description
        options  = {"temperature": 0.0, "num_predict": max_tokens, "num_ctx": ctx_size}

        # Both models stay in VRAM (OLLAMA_MAX_LOADED_MODELS=2)
        _ka = -1
        ...
        if _OLLAMA_LIB_OK:
            for attempt in range(1, 4):
                try:
                    ...
                    print(f"[LLM ERR] attempt {attempt} model={model}: {_e}\n{_tb.format_exc()[:300]}")
                    err_str = str(_e).lower()
                    if "connection refused" in err_str or "disconnected" in err_str:
                        # Ollama crashed — wait for it to recover
                        time.sleep(5 * attempt)
                    elif attempt == 1 and model != self.model:
                        model = self.model  # retry with main model
                    if attempt == 3:
                        return ""
                    time.sleep(2)
            return ""
        ...
        payload = {"model": model, "stream": False, "options": options,
                   "messages": messages, "keep_alive": _ka}
        for attempt in range(1, 4):
            try:
                resp = _get_session().post(url, json=payload, timeout=60)
                resp.raise_for_status()
                ...
                _tls.s = None
                if attempt == 1 and model != self.model:
...
class Watchdog:
    ...
    def __init__(self, log_fn=None, stat_fn=None):
        ...
        self._reason = None
        self._max_workers = 210          # will be set by pipeline
...
def pipeline(config: dict, q: queue.Queue):
    ...
    try:
        ...
        if gsm_list_file and os.path.isfile(gsm_list_file):
            ...
            # Find next available subset index (GSM_subset_1, _2, _3 ...)
            _subset_idx = 1
            while os.path.isdir(os.path.join(harmonized_dir,
        ...
        num_parallel  = config.get("num_workers") or auto_total
        free_gb       = psutil.virtual_memory().available / 1e9
        ...
        if True:  # was: if scratch_mode:
            ...
            def _extract_one_gsm(args):
                ...
                # Build prompt — proven working format (Title/Source/Char)
                ...
                full_prompt = (_EXTRACTION_PROMPT_TEMPLATE
                    .replace("{TITLE}", _title)
                    .replace("{SOURCE}", _source)
                    .replace("{CHAR}", _char))
                if _treat:
                ...
                if _desc:
                    full_prompt += f"\nDescription: {_desc}"
                # GSE experiment context — treatment often only at experiment level
                _gse_info = gse_meta.get(gse_, {})
                _gse_ctx_parts = []
                if _gse_info.get("title"):
                    _gse_ctx_parts.append(f"Experiment: {_gse_info['title'][:120]}")
                if _gse_info.get("summary"):
                    _gse_ctx_parts.append(f"Summary: {_gse_info['summary'][:250]}")
                if _gse_ctx_parts:
                    full_prompt += "\n" + "\n".join(_gse_ctx_parts)
                text = ""
                for _scratch_attempt in range(3):
                    text = worker_._llm_with_model(
                        full_prompt, model=EXTRACTION_MODEL,
                        max_tokens=200, system="")
                    if text:
                        break
                    _wait = 5 * (_scratch_attempt + 1)
                    log(f"  [P1 WARN] {gsm_}: empty response "
                        f"(attempt {_scratch_attempt+1}/3) — waiting {_wait}s")
                    if not ollama_server_ok(ollama_url):
                        log(f"  [P1 WARN] Ollama unreachable — waiting {_wait}s")
                    time.sleep(_wait)
                if not text:
                    log(f"  [P1 WARN] {gsm_}: failed after 3 attempts — "
                        f"GSE rescue will handle in Phase 2")
                else:
                    log(f"  [P1 RAW] {gsm_}: {repr(text[:150])}")
                result = _parse_json_extraction(text, _cols)
                return gsm_, result
            ...
            try:
                log(f"  Pre-loading {model} for Phase 2 collapse …")
                if _OLLAMA_LIB_OK:
                    _ollama_lib.chat(model=model,
                                     messages=[{"role":"user","content":"1"}],
                                     options={"num_predict":1,"num_ctx":512},
                                     keep_alive=-1, stream=False)
                log(f"  {model} loaded — both models in VRAM for Phase 2")
            except Exception as _sw:
                log(f"  [WARN] Model pre-load: {_sw} — continuing anyway")
