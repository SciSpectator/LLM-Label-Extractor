#!/usr/bin/env python3
"""
Terminal batch runner for llm_extractor.py — v4 (2026-04-08)

Architecture:
  Phase 1:  Raw extraction — extract Tissue/Condition/Treatment
            from GSM metadata + GSE context using per-label agents.
  Phase 1b: NS inference — for fields still NS after Phase 1,
            re-infer from GSE experiment description. Single LLM call.
  Phase 2:  Collapse — map raw labels to cluster names via
            Memory Agent (clusters.db).

Supported models:
  - gemma2:2b  (default) — 2 GB VRAM, ~174 ms/sample, stable
  - gemma4:e2b (optional) — 7.2 GB VRAM, ~0.5s/call with think=false, better reasoning

To switch models, change MODEL and EXTRACTION_MODEL below.
...
MIN_SAMPLES   = 5
MODEL         = G.DEFAULT_MODEL        # gemma4:e2b (default)
EXTRACTION_MODEL = G.EXTRACTION_MODEL  # gemma4:e2b — per-label agents with think=false
...
def main():
    ...
    try:
        ...
        # Move GPL10558 to front so it resumes first
        csv_platforms.sort(key=lambda p: (
            0 if p["gpl"] == "GPL10558" else 1,
            -p["sample_count"]
        ))
